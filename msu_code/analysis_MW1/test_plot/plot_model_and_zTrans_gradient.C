...
TF1 * make_simulated_gradient(double * coefficients, double * bfield, int pole)
{
	TF1 * func = new TF1("func", func_EngeProd, -1, 1, NP);
    for (int i = 0; i < (NP - 1)/2; ++i)
    {
...
double get_chi_squared(TF1 * func, double * z, double * b)
{
...
and sums them together. */
	...
	for (int i = 0; i < Nz; ++i)
	{
		double func_value = func -> Eval(z[i]);
		double residual   = pow(func_value - b[i],2);
		chi_squared = chi_squared + residual;
	}
	return chi_squared;
}

int plot_model_and_zTrans_gradient()
{
/* 
- Plot b_n??_m0.txt gradients for n=2 and n=6 with model using the 
  initial parameters (i.e. those used to simulate the data)
  
  FITS ARE NOT DONE IN THIS CODE.
*/
    //char f_dir[512]="/projects/aris/MP/field_params_DEV/param_files_2016/FSQ5/";
    TCanvas * can = new TCanvas("can", "Quadrupole and Duodecapole",600,800);
    can -> Divide(1,2);
    char f_dir[512]="";
    ...
    int pole = 2;
        ...
        graph -> SetTitle("n=2 Component;Z Position (m);Gradient");
        fit_graph(graph, fcoef, Bn, pole, fit);
        printf(" pole = %i,  LEFF = %lf\n", pole, LEFF);
        double residual_2 = get_chi_squared(func, read_data.z_pos, read_data.grad);
    pole = 6;
    	can -> cd(2);
        read_data = read_gradient_data(make_file_name(pole));
        TGraph * graph = graph_data(read_data);
        graph -> SetTitle("n=6 Component;Z Position (m);Gradient");
        fit_graph(graph, fcoef, Bn, pole, fit);
        printf(" pole = %i,  LEFF = %lf\n", pole, LEFF);
        double residual_6 = get_chi_squared(func, read_data.z_pos, read_data.grad);
/*
