...
void read_single_file_and_fit(double index = 0.10,int j=1, int i=1)
{
...
one to fit to fit with a constant effective legnth. */
    ...
    for (int i = 0; i < 10; ++i)
        read_single_file_and_fit(list_of_files[i],i,0);
    for (int j = 0; j < NP; ++j){
    ...
    chi_leff_calc = get_chi_squared(fitted_parameters_calc_leff, length_params, all_final_data.za, all_final_data.fa);
    
    for (int i = 0; i < 10; ++i)
        read_single_file_and_fit(list_of_files[i],i,1);
    for (int j = 0; j < NP; ++j){
        temp_array = scatter_data_from_array(all_coefficients, j);
        for (int i = 0; i < NPP; ++i)
    ...
    for (int i = 0; i < (NP*NPP); ++i)
        cout << fitted_parameters_calc_leff[i] << '\t'
        << fitted_parameters_constant_leff[i] << endl;
    cout << "chi leff constant = " << chi_leff_const << endl;
    cout << "chi leff calc     = " << chi_leff_calc  << endl;
    return 0;
}