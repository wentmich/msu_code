/* The file contains functions to be used in "field_fitter.C." It calculates 
residuals between two fields, one described by a file for a pre-simulated field
and another from a field simulated in the code from Enge parameters read in from
a second file. */
...
for the Enge coefficients (fcoef). NOTE: the large comment section at the bottom
of the function is necessary if there is no empty line at the bottom of the file 
"fname." In that case, the comment section should be included so that the last
line of the file can be read. If included otherwise, an error will occur due to 
a null pointer in "sscanf." */
...
FldData * File_Read_zScan_FldData(char *fname)
{
/* Reads in data from a file containing the data for a previously simulated or
experimental field. It places it into a FldData structure array and then outputs
that array. */
...
{
/* main function to analyze the error in the data through various methods. This
fully simulates both the model and the fitting field. */
...
double field_residual_calculator_from_COSY_field(char * field_file, char * fname_fit)
{
